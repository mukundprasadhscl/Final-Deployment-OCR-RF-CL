            ...
            if processed_data and processed_data.extracted_info:
                ...
                if st.session_state.edit_resume:
                    ...
                    with tab7:
                        ...
                        # Add a new skill category functionality
                        ...
                        # Add a button that's always visible
                        add_category_button = st.button("Add Skill Category")

                        # Then modify the condition to check for button press
                        if add_category_button and new_category and new_skills:
                            # Get the existing technical skills or initialize an empty dict
                        ...
                        # Define the order of skill categories (including any newly added ones)
                        skill_categories = list(extracted_info['skills'].get('technical', {}).keys())

                        # Additional predefined categories to ensure all original categories are present
                        predefined_categories = [
                            "Programming Languages", "Scripting Languages", "Tools and Frameworks",
                            "Databases", "Monitoring Tools", "Version Controllers", "Operating Systems",
                            "Cloud", "DevOps", "Infrastructure as Code (IAC)", "Automation Tools",
                            "Data Visualization/Report Tools", "Project Management Tools",
                            "Full Stack Development", "Mobile App Development", "IDEs",
                            "Markup Languages", "Machine Learning/AI", "Others"
                        ]
                        ...
                        # Iterate through categories in rows of 3
                        for i in range(0, len(skill_categories), 3):
                            ...
                            # Process second category in the row
                            if i + 1 < len(skill_categories):
                                with cols[1]:
                                    category = skill_categories[i + 1]
                                    skills = technical_skills.get(category, [])
                                    skills_str = ", ".join(skills) if skills else ""
                                    edited_skills_str = st.text_area(
                                        f"{category} Skills",
                                        skills_str,
                                        key=f"skills_{category}_1"
                                    )
                                    # Process edited skills
                                    edited_skills = [
                                        skill.strip()
                            ...
                            # Process third category in the row
                            if i + 2 < len(skill_categories):
                                with cols[2]:
                                    category = skill_categories[i + 2]
                                    skills = technical_skills.get(category, [])
                                    skills_str = ", ".join(skills) if skills else ""
                                    edited_skills_str = st.text_area(
                                        f"{category} Skills",
                                        skills_str,
                                        key=f"skills_{category}_2"
                                    )
                                    # Process edited skills
                                    edited_skills = [
                                        skill.strip()
                        ...
                        extracted_info['skills']['soft'] = [
                            skill.strip()
                            for skill in edited_soft_skills_str.split(',')
                            if skill.strip()
                        ]
                    # Certifications Tab
                    with tab8:
